...
public class Glavna {
	...
	public static void main(String[] args) {
		...
		LjudskiResursi<Klijent> klijenti = new LjudskiResursi<>();
		LjudskiResursi<Zaposlenik> zaposlenici = new LjudskiResursi<>();
		LjudskiResursi<Osoba> osobe = new LjudskiResursi<>();
		Scanner s = new Scanner(System.in);
		System.out.println("UNOS PODATAKA:");
		LOGGER.info("Zapoèet unos podataka");
		
		for(int i = 0, k = 0, z = 0; i < BROJ_OSOBA; i++)
		{
			...
			if(i < Tvrtka.BROJ_KLIJENATA)
			{
				...
				osobe.add(i, unesiPodatkeKlijenta(s));
				klijenti.add(k, (Klijent)osobe.get(i));
				LOGGER.info("Unesen klijent " + osobe.get(i).getPrezime() + " " + osobe.get(i).getIme());
	            k++;
			}
			...
			else if(i >= Tvrtka.BROJ_KLIJENATA & i < (Tvrtka.BROJ_KLIJENATA + Tvrtka.BROJ_ZAPOSLENIKA))
			{
				...
				osobe.add(i, unesiPodatkeZaposlenika(s));
				zaposlenici.add(z, (Zaposlenik)osobe.get(i)); 
				LOGGER.info("Unesen zaposlenik " + osobe.get(i).getPrezime() + " " + osobe.get(i).getIme());
				z++;
			}	
		...
		Collections.sort(osobe.getListaLjudskihResursa(),(o1, o2) -> o1.getPrezime().compareTo(o2.getPrezime()));
		...
		tvrtka.setKlijenti(klijenti.getListaLjudskihResursa());
		tvrtka.setZaposlenici(zaposlenici.getListaLjudskihResursa());
		...
		osobe.getListaLjudskihResursa().stream()
                                       .forEach(o -> {
                                    	             System.out.println("Prezime i ime: " +
                                                     o.getPrezime() + " " + o.getIme());
                                    	             LOGGER.info("Ispis osobe " + o.getPrezime() +
                                    	             " " + o.getIme());
        ...
        BigDecimal ukupnaCijenaSvihUsluga= izracunajCijenuUsluga(tvrtka);
		System.out.println("Ukupna cijena svih obavljenih usluga: " + ukupnaCijenaSvihUsluga + " KN");
        	...
        	for(int i = 0, brojAlarma = brojUsluga; i < brojAlarma; i++)
        	{
        		...
        		catch (AlarmPredIstekomException e) 
        		{
        			...
        			if(odabir.equals("DA"))
        			{
        				...
        				zaposlenikBr = unesiCijeliBroj("Odabir >> ", s, tvrtka.getZaposlenici().size());
	...
	public static void unesiUsluge(MaloprodajnaTvrtka tvrtka, int brUsluga, Scanner scanner){
        ...
        for(int i = 0; i < brUsluga; i++)
        {
    		...
    		tvrtka.getArtikli().stream()
    		                   .forEach(a -> {
    			                             System.out.println((tvrtka.getArtikli().indexOf(a) + 1 ) +
    			                                    		     "." + " ARTIKL:");
    		...
    		ukupnaCijena = ukupnaCijena.add(usluga.prodajaArtikala(brojArtikala, brArtikala -> {
    			usluga.setUslugaObavljena(true);
    			usluga.setUslugaNaplacena(true);
    			return (usluga.getCijenaUsluge().multiply(new BigDecimal(brojArtikala)));
    		}));
    		LOGGER.info("Ukupna cijena svih artikala: " + ukupnaCijena + " KN");
    		
    		usluga.setCijenaUsluge(ukupnaCijena);
    		usluge.add(i, usluga);
        }
        tvrtka.setUsluge(usluge);
        tvrtka.setAlarmi(alarmi);
        LOGGER.info("Postavljeni alarmi za cijelu tvrtku");
        System.out.println("Ukupna cijena prodanih artikala: " + ukupnaCijena + "KN");
	...
	{
		Optional<Alarm> alarm1 = Optional.ofNullable(alarm);
		if(alarm1.isPresent())
	    {
		    if(alarm1.get().getVrijemeAlarma().isAfter(LocalDateTime.now()))
		    {
			throw new  AlarmPredIstekomException();
		    }
				
		    else if(alarm1.get().getVrijemeAlarma().equals(LocalDateTime.now()) ||
			alarm1.get().getVrijemeAlarma().isBefore(LocalDateTime.now()))
		    {
			    if(alarm1.get().getStatusAlarma() == true)
				{
				alarm1.get().setStatusAlarma(false);
	...
	/**
	 * Raèuna i vraæa ukupnu cijenu svih obavljenih usluga.
	 * Kao argument prima referencu na objekt klase Tvrtka
	 * @param tvrtka referenca na tvrtku
	 * @return ukupna cijena svih obavljenih usluga
	 */
	public static BigDecimal izracunajCijenuUsluga(Tvrtka tvrtka)
	{
		BigDecimal cijenaSvihUsluga = new BigDecimal(tvrtka.getUsluge()
				                         .stream()
                                         .mapToInt(u -> u.getCijenaUsluge().intValue()) 
                                         .sum());
                                         
		return cijenaSvihUsluga;
